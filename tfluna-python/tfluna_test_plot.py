...
def set_baudrate(baud_indx=4):
    ...
    time.sleep(0.1) # wait to settle
    ser_new =serial.Serial("/dev/ttyUSB0", baudrates[baud_indx],timeout=0) # new serial device
...
prev_ser = serial.Serial("/dev/ttyUSB0", baudrates[prev_indx],timeout=0) # mini UART serial device
